...
- 如果文章是"每日大事件""周报""科技热点"等汇总类文章，应按其中的独立新闻逐条提取，不要合并为一条
- 每条独立新闻都应保留原文章的 URL 作为来源
- 同一具体事件在同一篇文章中只提取一次，不要重复

## 链接规则

- 所有链接必须来自原始收集的文章数据，不要虚构或猜测 URL
- 如果不确定某条新闻的来源链接，宁可不提供链接，也不要编造
- 微信公众号文章链接格式为 https://mp.weixin.qq.com/s/xxxxx，不要编造这个格式的 URL

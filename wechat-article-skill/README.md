...
1. **双轨标题**：`{当天最有关注度的事件+影响/悬念/冲突}｜M.D数据中心、算力、AI日报`（不含 emoji）
2. **今日判断**：首屏先给一句当天核心判断，再列3个变化信号
3. **🔥 今日重点**（慢热点，做深）：最多3条，含"为什么重要"、"这意味着"、"受益方"、"承压方"、"后续观察"
4. **⚡ 一分钟速览**（快资讯，做极简）：表格形式，列：动态概要 | 影响度 | 原文链接
5. **关注引导语**：文末留存卡片，引导关注

### 增长优化规则

- Agent 生成的标题、今日判断、为什么重要、受益方/承压方、后续观察必须逐条网络搜索验证，优先使用官方公告、监管/交易所文件、公司新闻稿和权威媒体。
- 每个核心观点至少有1个可靠来源支撑；融资金额、政策、订单、投产、芯片/AI模型、公司战略等高时效内容优先用2个来源交叉确认。
- 无法验证或来源冲突的观点不得写入标题和今日判断；只能找到二手消息时，必须降级语气或删除。
- 标题要有打开理由，但不能标题党；前半句优先控制在28个中文字符以内。
- 摘要保留事实链条，判断部分回答"谁受益、谁承压、下一步看什么"。
- 一分钟速览只保留有明确产业影响的新闻，弱新闻宁可删除。
